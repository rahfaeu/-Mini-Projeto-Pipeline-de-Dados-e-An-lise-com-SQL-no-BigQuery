...
SELECT Nome_Cliente
FROM `t1engenhariadados.tutor_rafael_trilha_3.Clientes`
WHERE Estado_Cliente = 'SP';

-- Pergunta 2: Quais produtos pertencem à categoria 'Ficção Científica'?
SELECT Nome_Produto
FROM `t1engenhariadados.tutor_rafael_trilha_3.Produtos`
...
SELECT
    ...
    P.Nome_Produto,
    V.Data_Venda
FROM `t1engenhariadados.tutor_rafael_trilha_3.Vendas` AS V
JOIN `t1engenhariadados.tutor_rafael_trilha_3.Clientes` AS C ON V.ID_Cliente = C.ID_Cliente
JOIN `t1engenhariadados.tutor_rafael_trilha_3.Produtos` AS P ON V.ID_Produto = P.ID_Produto
...
CREATE OR REPLACE VIEW `t1engenhariadados.tutor_rafael_trilha_3.v_relatorio_vendas_detalhado` AS
SELECT
    V.ID_Venda,
    V.Data_Venda,
    C.Nome_Cliente,
...
SELECT * FROM `t1engenhariadados.tutor_rafael_trilha_3.v_relatorio_vendas_detalhado` WHERE Estado_Cliente = 'RJ';
